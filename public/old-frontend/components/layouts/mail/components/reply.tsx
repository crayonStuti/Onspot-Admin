...
import { emailMessages, getCurrentSelectedEmail, getSelectedEmailData } from './mail-list-messages';
...
export function Reply({ onOpenChange, mode = 'reply' }: ReplyProps) {
	...
	const selectedEmailData = getSelectedEmailData(selectedEmailId) || emailMessages[0];
	const replyPlaceholder = mode === 'forward'
		? 'Enter email'
		: (selectedEmailData?.sender ? `@${selectedEmailData.sender.toLowerCase()}.com` : 'Enter email');
