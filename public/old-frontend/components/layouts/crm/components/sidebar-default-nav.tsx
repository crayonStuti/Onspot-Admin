...
import type { NavItem } from '@/app/crm/config/types';
      ...
      >
        {filteredNavItems.map((item) => {
          if (item.items && item.items.length > 0) {
            ...
            if (sidebarCollapse) {
              return (
                <AccordionMenuItem key={item.id} asChild value={item.id}>
                  <div>
                    <NestedItemCollapsed item={item} />
                  </div>
                </AccordionMenuItem>
              );
            }
            ...
            return (
              <AccordionMenuSub key={item.id} value={item.id}>
                <AccordionMenuSubTrigger>
                  {item.icon && <item.icon />}
                  <span>{item.title}</span>
                </AccordionMenuSubTrigger>
                <AccordionMenuSubContent parentValue={item.id} type="single" collapsible>
                  {item.items.map((subItem) => (
                    <AccordionMenuItem key={subItem.id} asChild value={subItem.path || subItem.id}>
                      <div>
...
function NestedItemCollapsed({ item }: { item: NavItem }) {
  const { sidebarCollapse } = useLayout();

  if (!sidebarCollapse) return null;

  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div className="cursor-pointer flex items-center justify-center size-full">
          <Tooltip delayDuration={500}>
            <TooltipTrigger asChild>
              <span>{item.icon && <item.icon />}</span>
            </TooltipTrigger>
